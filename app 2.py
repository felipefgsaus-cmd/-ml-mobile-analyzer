...
<a class="btn green" href="/prompt">Gerar Prompt Mestre V2</a>
</div>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Prompt Mestre V2 - ML Analyzer V8</title>
<style>
body{font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f5f5f5;color:#202020;margin:0}
.wrap{max-width:1120px;margin:auto;padding:16px}
.card{background:#fff;border:1px solid #e1e1e1;border-radius:16px;padding:18px;margin:14px 0}
...
<body><div class="wrap">
<h1>Prompt Mestre V2 — Conversão</h1>
...
"""
    ...
    data="\n\n".join(blocks)
    return f"""ATUE COMO UM ESTRATEGISTA SÊNIOR DE MARKETPLACES ESPECIALIZADO EM MERCADO LIVRE, INTELIGÊNCIA COMPETITIVA, SEO, COPYWRITING DE PERFORMANCE, MERCHANDISING DIGITAL, CRO (OTIMIZAÇÃO DE CONVERSÃO), REDUÇÃO DE OBJEÇÕES E PREVENÇÃO DE DEVOLUÇÕES.
...
def export_prompt():
    ...
    return Response(prompt.encode("utf-8"),mimetype="text/plain; charset=utf-8",
                    headers={"Content-Disposition":"attachment; filename=ml_v8_prompt_mestre_v2.txt"})
