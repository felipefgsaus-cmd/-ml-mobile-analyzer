...
<p class="muted">API oficial + catálogo + reviews + perguntas + ofertas relacionadas + fotos públicas do anúncio.</p>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Prompt Mestre - ML Analyzer+ Prompt Superior</title>
<style>
body{font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f5f5f5;color:#202020;margin:0}
.wrap{max-width:1120px;margin:auto;padding:16px}
.card{background:#fff;border:1px solid #e1e1e1;border-radius:16px;padding:18px;margin:14px 0}
...
<p class="muted">Gerado localmente pelo ML Analyzer+ Prompt Superior usando os dados já coletados. Nenhuma API de IA é necessária.</p>
<div class="card">
...
def collect(rec):
    ...
    # Fotos e catalog_product_id são dados públicos do anúncio.
    # Não enviamos Authorization aqui: alguns anúncios de terceiros retornam 403 quando
    # consultados com o token de outra conta.
    st,item_data=api_get(f"/items/{item_id}",auth=False)
    if st==200 and isinstance(item_data,dict):
        setf(fields,"item_pictures",item_data.get("pictures"),f"Mercado Livre /items/{item_id} (público)")
        catalog_pid=item_data.get("catalog_product_id")
        if catalog_pid and catalog_pid not in pids:
            pids.append(catalog_pid)
...
@app.route("/health")
def health():
    return {"ok":True,"version":"8.1-stable-prompt-v2.1-public-photos","mode":"web"}
