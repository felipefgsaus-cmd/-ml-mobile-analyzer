...
<p class="muted"><b>Coleta estável + fotos em cascata + Prompt Mestre V2.1</b></p>
...
<div class="metric"><b>Fotos encontradas</b><span>{{ (r.fields.item_pictures.value|length if r.fields.item_pictures else 0) + (r.fields.pictures.value|length if r.fields.pictures else 0) }}</span><div class="src">anúncio={{ r.fields.item_pictures.value|length if r.fields.item_pictures else 0 }} · catálogo={{ r.fields.pictures.value|length if r.fields.pictures else 0 }}</div></div>
<div class="metric"><b>Diagnóstico fotos /items</b><span>{{ r.photo_debug.item_status if r.photo_debug else "—" }}</span><div class="src">pictures={{ r.photo_debug.item_pictures_count if r.photo_debug else "—" }} · catalog_product_id={{ r.photo_debug.catalog_product_id if r.photo_debug and r.photo_debug.catalog_product_id else "nenhum" }}</div></div>
</div>

{% if r.photo_debug %}
<details style="margin:10px 0"><summary><b>Detalhes do diagnóstico de fotos</b></summary>
<pre class="code">{{ r.photo_debug|tojson(indent=2) }}</pre>
</details>
{% endif %}
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Prompt Mestre - ML Analyzer FINAL</title>
<style>
body{font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f5f5f5;color:#202020;margin:0}
.wrap{max-width:1120px;margin:auto;padding:16px}
.card{background:#fff;border:1px solid #e1e1e1;border-radius:16px;padding:18px;margin:14px 0}
...
<p class="muted">Gerado localmente pelo ML Analyzer FINAL usando os dados já coletados. Nenhuma API de IA é necessária.</p>
<div class="card">
...
def _picture_urls(record, key, limit=12):
    pics = _field_value(record, key) or []
    urls=[]
    for pic in pics:
...
Fotos do anúncio: {len(_field_value(r,'item_pictures') or [])}
URLs das fotos do anúncio: {_compact(_picture_urls(r,'item_pictures'),3500)}
Fotos do catálogo: {len(_field_value(r,'pictures') or [])}
URLs das fotos do catálogo: {_compact(_picture_urls(r,'pictures'),3500)}
Total de fotos encontradas: {len(_field_value(r,'item_pictures') or []) + len(_field_value(r,'pictures') or [])}
...
def collect(rec):
    ...
    photo_debug={
        "item_status": None,
        "item_response_type": None,
        "item_pictures_present": False,
        "item_pictures_count": 0,
        "catalog_product_id": None,
        "product_ids_from_input": list(pids),
        "item_error": None,
        "catalog_checks": []
    }
    ...
    if not item_id:
        return {
            ...
            "fields":fields,"other_offers":[],"photo_debug":photo_debug,
    ...
    # Diagnóstico temporário da resposta de fotos do próprio anúncio.
    st,item_data=api_get(f"/items/{item_id}",auth=True)
    photo_debug["item_status"] = st
    photo_debug["item_response_type"] = type(item_data).__name__
    if isinstance(item_data,dict):
        item_pics=item_data.get("pictures")
        photo_debug["item_pictures_present"] = "pictures" in item_data
        photo_debug["item_pictures_count"] = len(item_pics) if isinstance(item_pics,list) else 0
        photo_debug["catalog_product_id"] = item_data.get("catalog_product_id")
        photo_debug["item_keys_sample"] = sorted(list(item_data.keys()))[:40]
        if st != 200:
            photo_debug["item_error"] = item_data.get("message") or item_data.get("error") or str(item_data)[:500]
        if st==200:
            setf(fields,"item_pictures",item_pics,f"Mercado Livre /items/{item_id}")
    elif st != 200:
        photo_debug["item_error"] = str(item_data)[:500]
    ...
    for pid in pids[:4]:
        ...
        check={"product_id":pid,"status":st,"pictures_present":False,"pictures_count":0,"error":None}
        if isinstance(p,dict):
            pp=p.get("pictures")
            check["pictures_present"] = "pictures" in p
            check["pictures_count"] = len(pp) if isinstance(pp,list) else 0
            if st != 200:
                check["error"] = p.get("message") or p.get("error") or str(p)[:500]
        elif st != 200:
            check["error"] = str(p)[:500]
        photo_debug["catalog_checks"].append(check)
    ...
    return {
        ...
        "fields":fields,"other_offers":other_offers,"photo_debug":photo_debug,"_collected_at_unix":int(time.time())
    }
