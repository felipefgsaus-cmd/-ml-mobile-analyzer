...
def collect(rec):
    ...
    # Fotos do próprio anúncio. Esta chamada é independente do catálogo e funciona
    # também quando o anúncio não possui product_id de catálogo na URL.
    st,item_data=api_get(f"/items/{item_id}",auth=True)
    if st==200 and isinstance(item_data,dict):
        setf(fields,"item_pictures",item_data.get("pictures"),f"Mercado Livre /items/{item_id}")
